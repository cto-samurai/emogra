...
function putNewEmotion(meetingId, userId, emoNum, strength) {
    const emotionsRefStr = `/meetings/${meetingId}/users/${userId}/emotions/`
...
export default {
  ...
  methods: {
      onAgree(strength) {
          ...
          putNewEmotion(this.id, this.userId, 1, strength)
      },
      onDisagree(strength) {
          console.info("onDisagreed", strength)
          if (this.userId == "") return
          putNewEmotion(this.id, this.userId, 2, strength)
      },
      onConfused(strength) {
          console.info("onConfused", strength)
          if (this.userId == "") return
          putNewEmotion(this.id, this.userId, 3, strength)
      },
      onAbsent(strength) {
          console.info("onAbsent", strength)
          if (this.userId == "") return
          putNewEmotion(this.id, this.userId, 4, strength)
      },
  },
  async mounted() {
  }
